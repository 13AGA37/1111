...
class SushiRollController: 
    ...
    def get_rolls_for_restaurant(self): 
        return [{"name": roll.get_name(), "rice": roll.get_rice(), "nori": roll.get_nori(), 
                "fillings": roll.get_fillings(), "optional_toppings": roll.get_optional_toppings()} for roll in self.rolls] 

    def get_rolls_for_website(self): 
        return [{"name": roll.get_name(), "description": f"{roll.get_rice()} with {', '.join(roll.get_fillings())}", 
                "toppings": roll.get_optional_toppings()} for roll in self.rolls] 

    def update_roll(self, roll_name, new_rice=None, new_nori=None, new_fillings=None, user_role=None): 
        if user_role != "admin": 
            raise PermissionError("Access denied. You do not have permission to modify rolls.") 
...
# Creating a custom roll 
my_custom_roll = SushiRoll.create_custom_roll("My Custom Roll", "Sticky Rice", "Seaweed", ["Salmon", "Cream Cheese"], ["Chives"]) 
controller.add_roll(my_custom_roll) 

# Getting views 
restaurant_view = controller.get_rolls_for_restaurant() 
website_view = controller.get_rolls_for_website() 

# Updating a roll with permission check 
try: 
...
# Output views 
print("Restaurant View:", restaurant_view) 
print("Website View:", website_view)
